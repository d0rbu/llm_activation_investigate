...
def cosine_sim_heatmap(
    ...
    # Heatmap data
    cosine_sim_heatmaps = th.zeros(attn.shape[0], attn.shape[0], attn.shape[3])  # (L, L, T)
    ...
    # plot the heatmaps separately and save
    for base_layer in base_layers:
        fig, ax = plt.subplots()
        cax = ax.matshow(cosine_sim_heatmaps[base_layer].T, cmap="RdBu_r")
        fig.colorbar(cax)
